...
const AccData = (props) => {
    ...
    const updateProfileHandler = () => {
        ...
        let activeUserData = localData[`${props.activeUser}`];
        if (activeUserData) {
            setObj({
                email: accemail === "" ? activeUserData.email : accemail,
                name: accname === "" ? activeUserData.name : accname,
                password: accpwd === "" ? activeUserData.password : accpwd,
                phone: accphone === "" ? activeUserData.phone : accphone,
                profilePic: accpic === "" ? activeUserData.profilePic : accpic,
            });
            isUpdate(true);
            setModal(true);
        } else {
            // Handle the case where active user data is not available
    ...
    const deletephotoHandler = () => {
        ...
        setObj(prevState => ({
            ...prevState,
            profilePic: "", // Removing the profilePic property here
        }));

        // You can now use the updated state inside this block or log it in a useEffect
    };
            ...
            <div className={styles.container}>
                <div className={styles.accountdatacontainer}>
                    <div className={styles.imgcontainer}>
                        <img
                            src=
                            {accpic.length > 0 ? accpic : selectedDatatoShow?.profilePic || ""}
            ...
            {modal && (
                <div className={styles.modalouter}>
                    <div className={styles.modal}>
                        <FontAwesomeIcon onClick={() => setModal(false)} icon={faCircleXmark} />
